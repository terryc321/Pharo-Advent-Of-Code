...
```
SpWindowMorph allInstancesDo: [ :win | 
	 (win title = 'Playground') ifTrue: [ 
		win allMorphsDo: [ :morph | 			
			(morph balloonText) ifNotNil: [
				(morph balloonText includesSubstring: 'Refresh') ifTrue:[
					Transcript show: 'Refresh=>' ; show: morph balloonText string ; cr.
					morph sendClickEvent ]]]]].
...
we can get all isntances of StPlayground  
...
Here is a version searches for all morphs with Refresh in title - gets the morph and activates it
```
SpWindowMorph allInstancesDo: [ :win | 
	 (win title = 'Playground') ifTrue: [ 
		win allMorphsDo: [ :morph | 			
			morph balloonText ifNotNil: [
				morph balloonText contains:'Refresh' ifTrue:[
					Transcript show: 'Refresh=>' ; show: morph balloonText string ; cr.
					 ]]]]].
```
...
```
simulate a mouse click on a morph
ButtonValue (buttons:)Method to checkLeft (Red)4redButtonPressedRight (Yellow)2yellowButtonPressedMiddle (Blue)1blueButtonPressed
		...
		win allMorphsDo: [ :morph | 			
			(morph balloonText) ifNotNil: [
				(morph balloonText includesSubstring: 'Refresh') ifTrue:[
										| event |
					Transcript show: 'Refresh=>' ; show: morph balloonText string ; cr.
					"morph refreshContent ."
	    event := MouseButtonEvent new
        buttons: MouseEvent blueButton;      
		"blueButton redButton yellowButton 4 = red / left button" "1 = left button" 
		"how do we get morph position ??"
        position: (morph owner position)
        yourself.
     	morph mouseDown: event.
	      morph mouseUp: event.
	      morph click: event.
					 ]]]]].
		...
		win allMorphsDo: [ :morph | 			
			(morph balloonText) ifNotNil: [
				(morph balloonText includesSubstring: 'Refresh') ifTrue:[
					...
					"morph refreshContent ."
	    event := MouseButtonEvent new
        buttons: 1;                    "4 = red / left button" "1 = left button"
...
StPlayground allInstancesDo: [ :pg |
	Transcript show: pg printString ; cr.
].
...
StPlaygroundDoItCommand selectors. 
    when: CmCommand 
    do: [ :ann |
        StPlayground allInstancesDo: [ :pg |
            pg inspector ifNotNil: [ :ins |
...
SpMenuItemPresenter -- action attribute ---
...
StPlaygroundCommandTreeBuilder browse.
StPlaygroundInteractionModel browse.
...
See `StPlaygroundBindingsPresenter`
...
See `StPlaygroundPagesPresenter`
--------------------------------------------------
